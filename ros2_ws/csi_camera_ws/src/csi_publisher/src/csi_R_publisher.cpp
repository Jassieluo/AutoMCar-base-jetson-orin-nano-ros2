...
class csi_R_pubNode : public rclcpp::Node
{
    private:
        ...
        {
            capR>>imgR;
            if (imgR.size().height!=0)
            {
                // cv::resize(imgR, imgR, cv::Size(display_width, display_height));
                ImageRpublisher->publish(*cv_bridge::CvImage(std_msgs::msg::Header(), "bgr8", imgR).toImageMsg());
            }
        }

    public:
        csi_R_pubNode(const std::string pipline, const std::string nodeName, const std::string topicName, int cL=3):Node(nodeName)
        {
            capR = cv::VideoCapture(pipline, cv::CAP_GSTREAMER);
            ImageRpublisher = this->create_publisher<sensor_msgs::msg::Image>(topicName, cL);
            time_ = this->create_wall_timer(std::chrono::milliseconds(10), std::bind(&csi_R_pubNode::timer_publish, this));
        }
};
...
int main(int argc, char** argv)
{
    ...
    rclcpp::spin(std::make_shared<csi_R_pubNode>(pipline, "csi_R_puber", "csi_R_image"));
    rclcpp::shutdown(); 
}